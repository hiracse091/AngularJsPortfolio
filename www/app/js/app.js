    ...
    function routesConfig($stateProvider, $locationProvider, $urlRouterProvider, helper) {
            ...
            $stateProvider
                ...
                .state('app.dashboard', {
                    url: '/dashboard',
                    title: 'Dashboard',
                    controller: function($accountService, $syncService) {
                ...
                .state('app.create-job', {
                    ...
                    templateUrl: helper.basepath('create.job.html')
                })
                .state('app.detail-job', {
                    url: '/detail-job/:jobId',
                    title: 'Job Detail',
                    controller: 'JobReportController',
                    templateUrl: helper.basepath('report.engineering_report.html')
                })
                .state('app.shedule-job', {
                    url: '/schedule-job/:jobId',
                    title: 'Job Scheduling',
                    controller: 'JobScheduleController',
                    templateUrl: helper.basepath('schedule.job.html'),
                    resolve: helper.resolveFor('jquery-ui', 'jquery-ui-widgets', 'moment', 'fullcalendar')
                })
                .state('app.inspection-complete-confirm', {
                    url: '/inspection-complete/:activity/:scheduleId',
                    title: 'Confirm Inspection',
                    controller: 'CompleteInspectionController',
                    templateUrl: helper.basepath('complete.inspection.html')
                })
                .state('app.inspection-complete', {
                    url: '/inspection-complete/:activity/:scheduleId',
                    title: 'Comlete Inspection',
                    controller: 'CompleteInspectionController',
                    templateUrl: helper.basepath('complete.inspection.html')
                })
                .state('app.report-wrapping', {
                    url: '/report-wrapping/:jobId',
                    title: 'Wrapping',
                    controller: 'WrappingController',
                    templateUrl: helper.basepath('report.wrapping.html'),
                    resolve: helper.resolveFor('textAngular')
                })
                .state('app.roof_report', {
                    url: '/roof_report',
                    title: 'Roof Report',
                    templateUrl: helper.basepath('report.roof_report.html')
                })
                .state('app.leak_detection_report', {
                    url: '/leak_detection_report',
                    title: 'Leak Detection Report',
                    templateUrl: helper.basepath('report.leak_detection_report.html')
                })
                .state('app.cctv_report', {
                    url: '/cctv_report',
                    title: 'CCTV Report',
                    templateUrl: helper.basepath('report.cctv_report.html')
                })
                .state('app.engineering_report', {
                    url: '/engineering_report',
                ...
                .state('app.invoice', {
                    ...
                    templateUrl: helper.basepath('detail.invoice.html')
                })
                /*.state('app.detail-job', {
                    url: '/detail-job/:jobId',
                    title: 'Job Detail',
                    controller: 'JobReportController',
                    templateUrl: helper.basepath('report.job.html')
                })*/
            ;
...
(function() {
    ...
    function settingsRun($rootScope, $localStorage) {
        ...
        $rootScope.app = {
            name: 'AngularJS',
            description: 'AngularJS Portfolio',
    ...
    function UserBlockController($rootScope) {
        ...
        function activate() {
            $rootScope.user = {
                name: 'John',
                job: 'Administrator',
                picture: 'app/img/user/logo-single.png'
            };
...
(function() {
    ...
    calendar.$inject = ['$rootScope', '$mdDialog', '$schedule', '$fileNote'];

    function calendar($rootScope, $mdDialog, $schedule, $fileNote) {
        var directive = {
            link: link,
            restrict: 'EA'
        };
        ...
        function link(scope, element) {
            ...
            //     {
            //     title: 'Leak Detection Inspection (MTechDev)',
            //     start: new Date(y, m, 1),
            //     backgroundColor: '#f56954', //red 
            //     borderColor: '#f56954' //red
            // }
            $schedule.allEvents().then(function(schedules) {
                var demoEvents = schedules.map(function(s) {
                    return {
                        title: s.title,
                ...
                initCalendar(calendar, demoEvents, $rootScope.app.layout.isRTL, $mdDialog);
            });
        }
    }
     ...
     */
    function initCalendar(calElement, events, isRTL, $mdDialog) {
        ...
        calElement.fullCalendar({
            ...
            dayClick: function(date, jsEvent, view) {
                ...
                $mdDialog.show({
                    controller: function($scope, $mdDialog, $user, $schedule, $job, $stateParams, $fileNote, $accountService) {
                        var currentUser = $accountService.getCurrentUser();
                        $scope.schedule = null;

                        $user.getEstimators().then(function(users) {
                            $scope.estimators = users;
                        });
                        $scope.hide = function() {
                            $mdDialog.hide();
                        };
                        ...
                        $scope.answer = function(schedule) {
                            if (schedule) {
                                if (!schedule.estimator_id)
                                    return false;
                                schedule.job_id = $stateParams.jobId;
                                schedule.confirmed = 0;
                                schedule.completed = 0;
                                $schedule.insert(schedule).then(function() {
                                    $job.update($stateParams.jobId, {
                                        assigned_to: schedule.estimator_id
                                    }).then(function() {
                                        $fileNote.insert({
    ...
    function createDemoEvents() {
        ...
        return [{
            ...
            title: 'Roof Damage Inspection (MTechDev)',
            start: new Date(y, m, d - 5),
            end: new Date(y, m, d - 2),
            backgroundColor: '#f39c12', //yellow
            borderColor: '#f39c12' //yellow
        }, {
            title: 'CCTV Inspection (MTechDev)',
            ...
            borderColor: '#f39c12' //Success (green)
        }, {
            title: 'CCTV Inspection (MTechDev)',
